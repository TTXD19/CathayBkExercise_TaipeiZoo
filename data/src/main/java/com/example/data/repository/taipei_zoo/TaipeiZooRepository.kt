...
class TaipeiZooRepository @Inject constructor(
    ...
    override suspend fun getZonePlantDetails(offset: Int, limit: Int): DataResult<PlantDetailResp> {
        try {
            val response = taipeiZooDataSource.getZonePlanDetails(offset = offset, limit = limit)
            if (response.isSuccessful) {
                response.body()?.also {
                    return DataResult.Success(it.result)
                }
